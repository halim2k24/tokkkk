...
class HomeScreen:
    ...
    def draw_picking_box(self, frame, center_x, center_y, object_width, picking_box_size):
        # Move the picking box away from the center point by half the picking box size and object width
        # offset_distance = object_width / 2 + picking_box_size / 2
        offset_distance = object_width / 2  # Only consider the object width


        # Adjust start positions so that the first box is properly spaced away from the object
        start_x_0 = int(center_x + offset_distance)
        start_y_0 = int(center_y - picking_box_size / 2)

        # Draw the first picking box at 0 degrees
        cv2.rectangle(frame, (start_x_0, start_y_0),
                    (start_x_0 + picking_box_size, start_y_0 + picking_box_size),
                    (0, 255, 255), 2)  # Yellow color

        # Calculate the center point of the first picking box
        box_center_x_0 = start_x_0 + picking_box_size // 2
        box_center_y_0 = start_y_0 + picking_box_size // 2

        # Draw center point and coordinates for the first picking box
        cv2.circle(frame, (box_center_x_0, box_center_y_0), 3, (0, 255, 255), -1)  # Center point
        cv2.putText(frame, f"({box_center_x_0}, {box_center_y_0})", 
                    (box_center_x_0 + 5, box_center_y_0 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 1)

        # Now calculate the position for the second picking box at 180 degrees
        start_x_180 = int(center_x - offset_distance - picking_box_size)
        start_y_180 = int(center_y - picking_box_size / 2)

        # Draw the second picking box at 180 degrees
        cv2.rectangle(frame, (start_x_180, start_y_180),
                    (start_x_180 + picking_box_size, start_y_180 + picking_box_size),
                    (0, 255, 255), 2)  # Yellow color

        # Calculate the center point of the second picking box
        box_center_x_180 = start_x_180 + picking_box_size // 2
        box_center_y_180 = start_y_180 + picking_box_size // 2

        # Draw center point and coordinates for the second picking box
        cv2.circle(frame, (box_center_x_180, box_center_y_180), 3, (0, 255, 255), -1)  # Center point
        cv2.putText(frame, f"({box_center_x_180}, {box_center_y_180})", 
                    (box_center_x_180 + 5, box_center_y_180 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 1)

        # Optionally add a label "Pick" for each box
        cv2.putText(frame, "Pick 1", (start_x_0, start_y_0 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 1)
        cv2.putText(frame, "Pick 2", (start_x_180, start_y_180 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 1)
                        ...
                        for box in result.boxes:
                            ...
                            if inside_bearing:
                                if label == "ok":
                                    ...
                                    self.draw_picking_box(frame, center_x, center_y, object_width, picking_box_size=60)

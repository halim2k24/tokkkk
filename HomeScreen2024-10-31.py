...
class HomeScreen:
    ...
    def draw_picking_box(self, frame, center_x, center_y, object_width, picking_box_size):
        ...
        def calculate_positions(angle_0, angle_180):
            """Calculate the positions for both boxes at angles 0 and 180."""
            # Box at 0 degrees
            start_x_0 = int(center_x + object_width / 2)
            start_y_0 = int(center_y - picking_box_size / 2)

            # Box at 180 degrees
            start_x_180 = int(center_x - object_width / 2 - picking_box_size)
            start_y_180 = int(center_y - picking_box_size / 2)

            return (start_x_0, start_y_0, start_x_180, start_y_180)

        def draw_boxes(start_x_0, start_y_0, start_x_180, start_y_180):
            """Draw both boxes and their center points."""
            # Draw the box at 0 degrees
            cv2.rectangle(frame, (start_x_0, start_y_0),
                        (start_x_0 + picking_box_size, start_y_0 + picking_box_size),
                        (0, 255, 255), 2)  # Yellow color

            # Draw the box at 180 degrees
            cv2.rectangle(frame, (start_x_180, start_y_180),
                        (start_x_180 + picking_box_size, start_y_180 + picking_box_size),
                        (0, 255, 255), 2)  # Yellow color

            # Draw the center point of both boxes
            box_center_x_0 = start_x_0 + picking_box_size // 2
            box_center_y_0 = start_y_0 + picking_box_size // 2
            cv2.circle(frame, (box_center_x_0, box_center_y_0), 3, (0, 255, 255), -1)  # Center point 0

            box_center_x_180 = start_x_180 + picking_box_size // 2
            box_center_y_180 = start_y_180 + picking_box_size // 2
            cv2.circle(frame, (box_center_x_180, box_center_y_180), 3, (0, 255, 255), -1)  # Center point 180

        # Try 0-180 degree positioning first
        start_x_0, start_y_0, start_x_180, start_y_180 = calculate_positions(0, 180)

        if is_within_bearing_area(start_x_0, start_y_0, picking_box_size) and \
        is_within_bearing_area(start_x_180, start_y_180, picking_box_size):
            # If both boxes are within the area, draw them
            draw_boxes(start_x_0, start_y_0, start_x_180, start_y_180)
        else:
            print("Boxes at 0-180 degrees are out of bounds, trying 90-270 degrees...")

            # If 0-180 fails, try 90-270 degree positioning
            start_x_0, start_y_0 = int(center_x - picking_box_size / 2), int(center_y - object_width / 2 - picking_box_size)
            start_x_180, start_y_180 = int(center_x - picking_box_size / 2), int(center_y + object_width / 2)

            if is_within_bearing_area(start_x_0, start_y_0, picking_box_size) and \
            is_within_bearing_area(start_x_180, start_y_180, picking_box_size):
                draw_boxes(start_x_0, start_y_0, start_x_180, start_y_180)
            else:
                print("Unable to draw any picking box within the bearing area at 90-270 degrees.")

...
class HomeScreen:
    ...
    def draw_picking_box(self, frame, center_x, center_y, object_width, picking_box_size):
        ...
        # Define area box boundaries (example values, adjust as needed)
        area_x_min, area_y_min = 0, 0
        area_x_max, area_y_max = frame.shape[1], frame.shape[0]

        def is_within_area(x, y, size):
            return area_x_min <= x <= area_x_max - size and area_y_min <= y <= area_y_max - size

        if picking_angle == '0-180':
            angles = [0, 180]
        elif picking_angle == '90-270':
            angles = [90, 270]
        else:
            angles = [0, 180]  # Default to 0-180 if empty or invalid
        ...
        for angle in angles:
            ...
            # Check if the box is within the area
            if not is_within_area(start_x, start_y, picking_box_size):
                # Try alternative angles if out of bounds
                if picking_angle == '0-180':
                    angles = [90, 270]
                else:
                    angles = [0, 180]
            ...
            # Calculate the center point of the picking box
            box_center_x = start_x + picking_box_size // 2
            box_center_y = start_y + picking_box_size // 2

            # Draw center point and coordinates for the picking box

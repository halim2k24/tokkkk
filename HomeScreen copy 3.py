        ...
        if grab_result.GrabSucceeded():
            ...
            if None not in (self.start_x_one, self.start_y_one, self.end_x_one, self.end_y_one):
                ...
                if x1 < x2 and y1 < y2:  # Ensure the coordinates are valid
                    ...
                    print(f"Invalid crop area for Area One: ({x1}, {y1}), ({x2}, {y2})")
            ...
            if None not in (self.start_x_two, self.start_y_two, self.end_x_two, self.end_y_two):
                ...
                if x1 < x2 and y1 < y2:  # Ensure the coordinates are valid
                    # Crop the second image from Area Two
                    ...
                    print(f"Invalid crop area for Area Two: ({x1}, {y1}), ({x2}, {y2})")

            # Check if there are any crops to merge
            if crops_and_positions:
                # Merge the cropped images onto the RGB base image
                rgb_base_image = pil_image.convert("RGB")
